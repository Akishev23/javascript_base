...
const map = {
    ...
    render(snakePointsArray, foodPoint, snagPoint) {
        ...
        const snagCellKey = `x${snagPoint.x}_y${snagPoint.y}`;
        const snagCell = this.cells[snagCellKey];

        foodCell.classList.add('food');
        snagCell.classList.add('snag');

        this.usedCells.push(foodCell);
        this.usedCells.push(snagCell);
    },
};
...
const game = {
    ...
    reset() {
        ...
        this.snag.setCoordinates(this.getRandomFreeCoordinates())
        this.render();
        this.setScore();
    },

    render() {
        this.map.render(this.snake.body, this.food.getCoordinates(), this.snag.getCoordinates());
    },
    ...
    tickHandler() {
        ...
        if (this.snag.isOnPoint(this.snake.getNextHeadPoint())) {
            this.finish()
        }
    ...
    setScore() {
        const scoreId = document.getElementById('score')
            scoreId.innerHTML= `Общий счет: ${snake.getBody().length - 1}`;
    }
